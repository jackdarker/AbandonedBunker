...
window.gm.printMap=function(MapName,playerTile,reveal,visitedTiles) {
  ...
  for(el of visitedTiles) {
    ...
    _n= node.find('#'+el)[0];
    if(_n) {_n.removeClass('roomNotFound');_n.removeClass('roomFound');_n.addClass('roomVisited');}
  }
  _n= node.find('#'+playerTile)[0];
  if(_n) {_n.removeClass('roomNotFound');_n.removeClass('roomFound');_n.addClass('playerPosition');}
  node.addTo(draw);
}
/* 
